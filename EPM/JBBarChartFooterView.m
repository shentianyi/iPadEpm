...
+ (void)initialize
{
	...
	{
		kJBBarChartFooterPolygonViewDefaultBackgroundColor = [UIColor clearColor];
	}
}
...
- (void)layoutSubviews
{
    ...
    self.leftLabel.frame = CGRectMake(xOffset, yOffset+6.0, width, self.bounds.size.height);
    self.rightLabel.frame = CGRectMake(CGRectGetMaxX(_leftLabel.frame), yOffset+6.0, width, self.bounds.size.height);
}
